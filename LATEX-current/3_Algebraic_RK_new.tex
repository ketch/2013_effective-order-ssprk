...
concepts of this representation used to define effective order methods
...
Two Runge--Kutta methods are equivalent if they have the same values for these expressions.
...
The above operation is best understood via an example.
...
\colintodo{Move this stuff about $O(h^{q+1})$ later with the Result?}
For the method $P$ to be of order $q$, it must reproduce the exact solution to within $\mathcal{O}((\Dt)^{q+1})$.
