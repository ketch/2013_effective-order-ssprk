...
The Runge--Kutta methods can be viewed as an algebraic group in which the product
corresponds to composition.
Butcher \cite{Butcher1972} proposed the group $G$ of all real-valued maps on the
set of rooted trees.\davidtodo{Did he propose the group (whatever that means),
...
Two Runge--Kutta methods are equivalent if they have the same elementary weight values.
...
$BA$.\footnote{We write $BA$ to mean application of method $A$ followed by application of method $B$
...
\end{displaymath}
\davidtodo{The last equality in this formula is not actually equality, only equivalence up
to some order.}
