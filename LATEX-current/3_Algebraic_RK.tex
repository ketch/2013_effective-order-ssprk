...
\yiannistodo{Just to agree with the way we define functions in $G$.}
...
\footnote{We write $BA$ to mean application of method $A$ 
followed by application of method $B$ 
(following matrix and operator ordering convention) 
but when referring to products of elements of the Runge--Kutta group 
we use the reverse ordering ($\alpha\beta$) to match the convection in 
\cite[Theorem 384A]{Butcher2008_book}.}
...
discuss equivalence of methods up to a particular order of accuracy.
\davidtodo{You need to fix the writing so that the meaning is clear.}
\colintodo{double-check now}
...
\end{definition}
\davidtodo{Have we defined $E(t)$ somewhere?}
\colintodo{yes, briefly, added a note here too}
...
\davidtodo{I thought that we weren't using this approach anymore?}
...
freedom than the classical order conditions.\davidtodo{Don't we mean that the methods have more degrees of freedom?}
