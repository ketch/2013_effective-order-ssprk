\section{Runge--Kutta methods with effective order of accuracy}\label{sec:Algebraic_RK}
...
%	     \end{tabular}
%	     \vspace{5pt}
%	     \caption{Partitions of the tree $t_{11}$ to all possible subtrees $w$ 
...
\subsubsection{Classical and effective order}\label{subsubsec:Classical_effective_order}
...
%  For example, the forward Euler method with a step of size $-\Dt$ is also an
...
\end{definition}
The interested reader can read \cite{Butcher2008_book} for more details regarding 
...
second order.
\remark{The effective order conditions of the main method for the ``tall" trees 
...
on the tall trees.}\label{rem:talltrees}
