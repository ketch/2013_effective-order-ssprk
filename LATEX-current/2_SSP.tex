...
\cite{Shu/Osher:1988}\label{eq:HCL}
\begin{align}
...
Assume that the solution of \eqref{eq:HCL} satisfies a monotonicity 
...
\begin{definition}[Strong Stability Preserving]
	Suppose that the Forward Euler method applied to \eqref{eq:HCL} 
	is strongly stable in a norm, seminorm or convex functional $\|\cdot\|$, i.e.
	\begin{align*}
		\|\bm{u}^{n} + \Dt\bm{F}(\bm{u}^{n})\| \le \|\bm{u}^n\|, \; \forall \bm{u},
	\end{align*}
	under suitably-restricted time steps $0 \le \Dt \le \Dt_{\text{FE}}$,
	in a norm, seminorm, or convex functional  of interest.
...
\yiannistodo{I modified the definition.}
...
optimal explicit SSP Runge--Kutta methods of effective order. \davidtodo{Do you mean effective order RK methods?
