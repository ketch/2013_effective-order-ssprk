...
\begin{align}\label{eq:forwardEuler}
    \|\bm{u} + \Dt\bm{F}(t,\bm{u})\| \le \|\bm{u}\| \ \ \ \ \mbox{ for } 0\le\Dt\le \DtFE, \forall \bm{u}.
\end{align}
...
\davidtodo{Colin, the FE condition is always assumed to hold for all $u$, and $\DtFE$ must be
uniform.}
...
\begin{definition}[Strong Stability Preserving]
	...
	constant $\DtFE$ satisfy \eqref{eq:forwardEuler}.
