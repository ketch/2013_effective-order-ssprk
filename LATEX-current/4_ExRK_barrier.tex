...
\yiannistodo{I did some changes mostly related to your comments. Is it now ok?}
    ...
    \davidtodo{As we discussed, please make sure that the terminology "five-effective order" etc. does not appear anywhere in the paper.  I removed it from here.}\yiannistodo{I replaced those found by search command. l'll have it on mind when read the whole paper again.}
    ...
    \begin{equation}\label{eq:case_2}
    	...
    	\end{align*}
    	and hence\davidtodo{Please explain how you obtain \eqref{eq:case_2_b}}
    	\begin{equation}\label{eq:case_2_b}
            \beta_2\sum_{j \neq i}^s b_j v_j < \beta_2^2.
    \end{equation}
    Suppose that $v_{i} < 0$. By positivity of $\bm{b}$ we have\davidtodo{Explain how the next inequality is obtained.}
    \begin{align*}
    		\beta_2 = \sum_{j=1}^s b_j v_j < \sum_{j \neq i}^s b_j v_j,
    	\end{align*}
