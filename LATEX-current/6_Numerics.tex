...
Figure~\ref{fig:burgers_cont} shows that if $\sigma$ stays less the SSP 
...
This is $25\%$ larger the value SSP coefficient $\sspcoef = 1.95$.

\begin{figure}
    \centering
    \subfloat[$\sigma = 1.95$]{\label{fig:burgers_cont_a}
      \includegraphics[width=0.55\textwidth]{Pictures/burgers_cont_tvd}}
    \subfloat[$\sigma = 2.44$]{\label{fig:burgers_cont_b}
      \includegraphics[width=0.55\textwidth]{Pictures/burgers_cont_no_tvd}}
    ...
    Here $TV$ denotes the the $TV$-norm of the solution at the final time.
    Figure~\ref{fig:burgers_cont_b} shows a zoom in the region where 
    oscillations appear.
    A value greater the TV-norm of the initial solution 
    ($\text{TV}_{\text{initial}} = 1$) indicates a violation of the TVD condition.}
...
SSPRK($4,4,2$) method.
The solution consists of a rarefraction (i.e. an expansion fan ) at $x = 0.5$ and 
a contact discontinuity at $x=1.5$. 
Again we use $m = 200$ points in space and we compute the solution at 
roughly time $t = 0.3$.
When $\sigma$ is $22\%$ larger the value of SSP coefficient $\sspcoef = 0.88$, 
the total variation is first increased at time $t = 0.096$. 
However, any oscillations appeared do not propagate and are damped by 
the stopping procedure, thus at the final time the solution is total--variation 
preserving. 
Increasing $\sigma$ further will cause oscillations to be unbounded. 
In this case, $\sigma = 1.36$ appears to be the largest value of $\sigma$ 
for which the total variation has not increased at the final time $t = 0.3$.
This is $55\%$ larger the value of the SSP coefficient.
Figure~\ref{fig:burgers_discont_b} shows part of the solution exhibiting 
oscillations when $\sigma$ is $61\%$ larger the SSP coefficient.

\begin{figure}
    \centering
    \subfloat[$\sigma = 0.88$]{\label{fig:burgers_discont_a}
      \includegraphics[width=0.55\textwidth]{Pictures/burgers_discont_tvd.eps}}
    \subfloat[$\sigma = 1.41$]{\label{fig:burgers_discont_b}
      \includegraphics[width=0.55\textwidth]{Pictures/burgers_discont_no_tvd.eps}}
    \caption{Solution of Burgers' equation with discontinuous initial data, using a 
    $TM^{n-2}R$ scheme, where $M$ is SSPRK($4,4,2$) method. 
    The SSP coefficient is $ \sspcoef = 0.88$. 
    Figure~\ref{fig:burgers_discont_b} shows a zoom in the region where 
    oscillations appear.
    Here $TV$ denotes the the $TV$-norm of the solution at the final time.
    A value greater the TV-norm of the initial solution 
    ($\text{TV}_{\text{initial}} = 2$) indicates a violation of the TVD condition.}
...
since the dissipation of the solution is decreased. \yiannistodo{Is this explanation enough?}
