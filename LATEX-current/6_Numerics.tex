...
SSPRK($4,4,2$) method.
The solution consists of a rarefaction (i.e., an expansion fan) around $x = 0.5$ and
a contact discontinuity at $x=1.5$. 
Again we use $m = 200$ points in space and we compute the solution until
roughly time $t = 0.3$.
\colintodo{We want TVD over the entire time, so 22\% is the right number.  I will fix this tomorrow if I have time.}
...
\begin{figure}
    ...
    \subfloat[$\sigma = 0.88$]{\label{fig:burgers_discont_a}
      \includegraphics[width=0.5\textwidth]{Pictures/burgers_discont_tvd}}
    \subfloat[$\sigma = 1.41$]{\label{fig:burgers_discont_b}
      \includegraphics[width=0.5\textwidth]{Pictures/burgers_discont_no_tvd}}
