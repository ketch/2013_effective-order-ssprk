...
Having constructed a $TM^{n-2}R$ scheme, as explained in the previous 
section, we now numerically verify its properties.
...
\colintodo{I'm not sure this is a good idea: ESSPRK scheme already
 ...
 \yiannistodo{I think is the other way: ``methods" for individual $M,R,T$ 
 and ``scheme" for the whole thing.}
...
using an ESSPRK-scheme of effective order three and four and classical 
order two.
...
the results show that the ESSPRK-scheme attains an order of accuracy equal 
to the effective order of the main method.
