...
Additionally, we find a family of
methods with effective order four, for which $\ceff$ asymptotically approaches unity.
The family consists of second order methods with $s = n^2+1$ stages and 
SSP coefficient $n^2-n$.
It is convenient to express the coefficients in the Shu--Osher form
\textbf{\red [todo: citation]},
because of the sparsity of the $s+1 \times s$ matrices $\alpha$ and $\beta$.
...
In \cite[Section~6.2.2]{Gottlieb2011a}, a similar pattern was found
for SSPRK methods of fourth-order methods with $s=XX$ stages
