...
Specifically, we use \texttt{fmincon} with a sequential quadratic programming approach \cite{Ketcheson2008}.
...
\end{equation}
%and equivalently,
%\colintodo{Is this useful?  if useful, we should double-check that it is equivalent.}
...
% colin: no: beta_2 is different in each case
...
\begin{table}
	...
	\begin{tabular}{lcl}
  	...
  	and finishing methods $R$ and $T$, respectively.
...
Also note that for either case of effective order $q=3$ or $q=4$, the algebraic expressions on 
...
The starting and finishing schemes themselves are first order accurate, except when the main 
method is ESSPRK($s,4,3$), where 
they achieve second order accuracy. 
...
\begin{table}
    ...
    \caption{ESSPRK(4,4,2): an effective order four SSPRK method with
      ...
      %Fourth order effective SSP scheme where the main method $M$ is ESSPRK(4,4,2).}
