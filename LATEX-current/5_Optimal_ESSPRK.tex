...
by replacing now the order conditions $\Phi(K)$ with $\Phi_{q,p}(K)$,
representing the conditions for effective order
\davidtodo{$\Phi_{q,p}(K)$ does not appear in (2.4).}
...
have $\ceff < 1$. \davidtodo{This is known a priori.}
...
methods.
A consequence of the definitions of these method classes is that the 
...
%	or equal to that of SSPRK($s,q$) methods.
%\end{result}
\davidtodo{This is known a priori; it is a consequence of the definitions of these
method classes.}

Furthermore, useful upper bounds for the optimization problem of effective order 
SSP Runge--Kutta methods can be obtained by considering an important relaxation. 
...
example see \cite{Kraaijevanger1986,ketcheson2009a}. We then have the 
next result.
\begin{result}
...
\end{result}
\davidtodo{What is meant by ``the linear bound''?  I don't think that is a good
description of what is meant.  Furthermore, I don't think the reader will remember
what was mentioned briefly (and never again since) in Section 2.}
\yiannistodo{OK. I removed Result 5.1 and bring the linear bound here. If it is better now
we then remove it form sec2.}
...
For $7\le s\le 11$ stages, the SSP coefficient of ESSPRK($s,4,2$) and
